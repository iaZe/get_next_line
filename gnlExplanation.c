...
char    *ft_strjoin(char *left_str, char *buff) // 5
{
    size_t    i;
    size_t    j;
    char    *str;

    if (!left_str) // Faz uma verificação para saber se left_str é vazio
    {
        left_str = (char *)malloc(1 * sizeof(char)); // Caso seja, criamos um array na memória usando ele como array
        left_str[0] = '\0'; // incrementamos na posição zero um byte nulo
    }
    if (!left_str || !buff) // Caso ambas funções estejam vazias retornamos nulo
        return (NULL);
    str = malloc(sizeof(char) * ((ft_strlen(left_str) + ft_strlen(buff)) + 1)); // Alocamos na memoria uma array com tamanho de strlen dos parametros left_str e buff + 1
    ...
    if (left_str) // Caso não estejamos no final de left_str
        while (left_str[++i] != '\0') // Faremos um loop para concatenar tudo que estiver em left_str para str
            str[i] = left_str[i];
    while (buff[j] != '\0') // Após isso concatenamos tudo que estiver em buff para str (usamos 2 contadores pois os valores são separados)
        str[i++] = buff[j++];
    str[ft_strlen(left_str) + ft_strlen(buff)] = '\0'; // Adicionamos a str no tamanho de left_str + buff um byte nulo
    free(left_str); // Limpamos o valor de left_str
    return (str); // Retornamos str para função principal
}

char    *ft_get_line(char *left_str) // 6
{
    int        i;
    char    *str;

    i = 0; // iniciamos o i com valor de 0
    if (!left_str[i]) // Caso o valor de left_str no byte de i seja vazio
        return (NULL); // Retornamos nulo
    while (left_str[i] && left_str[i] != '\n') // Enquanto não chegar ao final de left_str ou chegar a uma quebra de linha
    ...
    while (left_str[i] && left_str[i] != '\n') // Enquanto não chegar ao final de left_str ou a uma quebra de linha
    {
        str[i] = left_str[i]; // Iremos copiar o valor de left_str para str
        i++;
    }
    if (left_str[i] == '\n') // Caso left_str esteja em uma quebra de linha
    {
        str[i] = left_str[i]; // Iremos incrementar a quebra de linha 1 vez
        i++;
    }
    str[i] = '\0'; // Incrementamos um byte nulo
    return (str); // Retornamos str para a função principal ** obs: não limpamos left_str dessa vez
}

char    *ft_new_left_str(char *left_str) // 7
{
    int        i;
    int        j;
    char    *str;
    ...
    while (left_str[i] && left_str[i] != '\n') // Fazemos novamente a verificação enquanto left_str não chegar ao final ou a uma quebra de linha
        i++; // Iremos incrementar a I
    if (!left_str[i]) // Caso não tenha nada em left_str no valor de i
    {
        free(left_str); // Limpamos o parametro
        return (NULL); // E retornamos nulo
    }
    str = (char *)malloc(sizeof(char) * (ft_strlen(left_str) - i + 1)); // Criamos um array alocado na memoria com o valor de left_str - i + 1
    if (!str) //Caso o alocamento seja falho
        return (NULL); // Retornamos nulo
    i++; // Incrementamos 1 a i
    j = 0; // Inciamos o parametro j com valor 0
    while (left_str[i]) // Enquanto left_str não for vazio
        str[j++] = left_str[i++]; //Vamos copiar o valor de left_str para str
    str[j] = '\0'; //Adicionamos um byte nulo ao final de str
    free(left_str); // Limpamos os dados de left_str
    return (str); // Retornamos str a função principal
}

char    *ft_read_to_left_str(int fd, char *left_str) // 2
{
    char    *buff;
    int        rd_bytes;
    ...
    while (!ft_strchr(left_str, '\n') && rd_bytes != 0) // Faz a busca por quebra de linha em left_str usando a função ft_strchar e enquanto rd_bytes não for zero mantemos o loop
    {
        ...
        left_str = ft_strjoin(left_str, buff); // Concatena na função de left_str usando os parametros recebidos por ft_strjoin
    }
...
char    *get_next_line(int fd) // 1
{
    ...
    left_str = ft_read_to_left_str(fd, left_str); // Invoca a função ft_read_to_left_str passando os argumentos para left_str
    if (!left_str) // Caso o alocamento para left_str dê errado, retornamos nulo
        return (NULL);
    line = ft_get_line(left_str); // Passamos para line o retorno da função de ft_get_line com o valor de left_str
    left_str = ft_new_left_str(left_str); // Passamos para left_str o novo valor que foi retornado de ft_new_left_str
